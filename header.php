<?php 
    session_start();
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>E-Visa Portal</title>
    <link rel="stylesheet" href="css-reset.css">
    <link rel="shortcut icon" type="image/png" href="Images/fevicon.png">
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
    <link rel="stylesheet" href="style.css">
    <link href="https://fonts.googleapis.com/css?family=Acme|Josefin+Sans:400,400i&display=swap" rel="stylesheet">

</head>
<body>
    

    <header>
        
        <a href="index.php">
            <img src="Images/globe-white.jpg" alt="Logo" width="65px" height="65px">  </a>
            <div class="logo"> <span class="highlight">
            E-VISA PROCESSING </span> & Follow Up System </div>


        <nav>
        
        <?php 
    if (isset($_SESSION['uId'])) {
        echo "<a href='application.php' class='mybutton'>Application</a>";
    }
    else {
        echo "<a href='signup.php' class='mybutton'>Signup</a>";
    }
    ?>

        <a href="about-evisa.php" class="mybutton"> About E-Visa </a>
        <a href="faq.php" class="mybutton"> FAQ </a>


    <?php 
    if (isset($_SESSION['uId'])) {
        echo "<a href='profile.php' class='mybutton' >Profile</a>";
    } 
    ?>

    </nav>
    </header>




   